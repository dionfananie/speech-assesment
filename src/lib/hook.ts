...
export function getLineHeightOfFirstLine(element: HTMLElement): number {
  const compStyles = window.getComputedStyle(element, '::first-line').textC;
  console.log({compStyles})

  return parseFloat(compStyles)
}
...
export function useHoveredParagraphCoordinate(
  ...
  const [coordinates, setCoordinates] = useState<HoveredElementInfo | null>(null)

  useEffect(() => {
    const handleMouseMove = (event: MouseEvent) => {
      event.preventDefault()
      const { clientX, clientY } = event
      const targetElement = parsedElements.find(element => {
        return isPointInsideElement({ x: clientX, y: clientY }, element)
      })
      ...
      if (targetElement instanceof HTMLElement) {
        ...
        setCoordinates({
          ...
          ...bounds,
          heightOfFirstLine: getLineHeightOfFirstLine(targetElement)
        })
      }
  ...
  return hoveredElement ? coordinates : null
}
